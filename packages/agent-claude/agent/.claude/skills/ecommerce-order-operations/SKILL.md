...
1. 用户提供订单号时，直接调用 `mcp__toolbox__get-ecommerce-order-detail`，不要先扫描订单列表。
2. 用户询问一段时间的订单时，调用 `mcp__toolbox__list-ecommerce-orders-in-window`，时间窗不超过 31 天且结果有界。
...
### mcp**toolbox**list-ecommerce-orders-in-window
