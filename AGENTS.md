...
- Agent CMD 运行 Node.js、pnpm 或 Turbo 命令前执行 `(command -v nvm >/dev/null && nvm use) || { CI=true pnpm install --frozen-lockfile && source ~/.zprofile; }`，随后用 `node --version` 确认是 `v24.*`；不使用默认的 Node 22。
- 无 TTY 的冻结安装若提示必须重建 `node_modules`，立即以 `CI=true pnpm install --frozen-lockfile` 重试；这是目录重建，不是依赖解析失败。不要删除 `node_modules`、移除 `--frozen-lockfile` 或修改锁文件。
- 用户可见文案默认中文；技术名词保留英文。
